...
estimators_predators <- data.frame(matrix(0,9,8))
names(estimators_predators)<-c("zone","model","a","alpha","b","c","d","sigma")
...
for (zone in 1:3){
  ...
  model3 <- lm(formula = P1t ~ P1t_1 + At_1 + St_1 + At_2 ,offset = P1t_1 )
  model4 <- lm(formula = P2t ~ P2t_1 + At_1 + St_1 + At_2 ,offset = P2t_1 )
  model5 <- lm(formula = P3t ~ P3t_1 + St + At_1 + St_1 ,offset = P3t_1 )
  ...
  estimators_predators[3*(zone-1)+1,8] <- summary.lm(model3)$sigma
  estimators_predators[3*(zone-1)+2,8] <- summary.lm(model4)$sigma
  estimators_predators[3*(zone-1)+3,8] <- summary.lm(model5)$sigma
  
  # we can end the zone's loop 
}

# we can register our data frame in csv 
write.csv (estimators_vole, "../data_supp/parameters_vole_step2.csv", row.names = T, quote = F)
write.csv (estimators_predators, "../data_supp/parameters_predators_step2.csv", row.names = T, quote = F)
...
parameters <- read.csv("../data_supp/parameters_vole_step2.csv", sep=",", header=T, dec=".",stringsAsFactors=FALSE)
parameters_pred <- read.csv("../data_supp/parameters_predators_step2.csv", sep=",", header=T, dec=".",stringsAsFactors=FALSE)
  ...
  for (zone in 1:3){
    ...
    for (i in 1:1000){
      ...
      if (scenario ==1 ||scenario ==3 ||scenario ==4 ||scenario ==6 ){
        P1t_1 <- P1t
        P1t <- a3 + (alpha3+1)*P1t +b3*At +c3*St + d3*At_1 + rnorm(1,0,sigma3) 
      }
      if (scenario ==1 ||scenario ==2 ||scenario ==5 ||scenario ==6 ){
        P2t <- a4 + (alpha4+1)*P2t +b4*At +c4*St +d4*At_1 + rnorm(1,0,sigma4)
      }
      # Spring
      St_2 <- St_1
      St_1 <- St
      ...
      if (scenario ==1 ||scenario ==2 ||scenario ==4 ||scenario ==7 ){
        P3t <- a5 + (alpha5+1)*P3t +b5*St +c5*At +d5*St_1 + rnorm(1,0,sigma5) 
      }
      # Autumn 
      At_2 <- At_1
      At_1 <- At 
...
write.csv (Vole, "../data_supp/vole_density_simulations_step2.csv", row.names = T, quote = F) 
write.csv (simulations, "../data_supp/simulations_indicators_step2.csv",row.names=T,quote=F)

#--------------------------------------------------------------------------------------------------------

# III Figures : direct and delayed density dependence + s-index 

simulations <- read.csv("../data_supp/simulations_indicators_step2.csv",sep=",", header=T, dec=".", stringsAsFactors=FALSE)
...
pdf("../Figures_supp/graph_triangle_step2.pdf",  
    width = 16, height = 12, 
    bg = "white",         
    colormodel = "cmyk")
...
pdf("../Figures_supp/graph_s_index_step2.pdf",  
    width = 12, height = 6, 
    bg = "white",         
    colormodel = "cmyk")
...
# data importation 
Vole <- read.csv("../data_supp/vole_density_simulations_step2.csv",sep=",", header=T, dec=".", stringsAsFactors=FALSE)
...
pdf("../Figures_supp/sim_north_step2.pdf",  
    width = 8, height = 18, 
    bg = "white",         
    colormodel = "cmyk")
...
# Opening the graphic device 
pdf("../Figures_supp/sim_east_step2.pdf",  
    width = 8, height = 18, 
    bg = "white",         
    colormodel = "cmyk")
...
# Opening the graphic device 
pdf("../Figures_supp/sim_west_step2.pdf",  
    width = 8, height = 18, 
    bg = "white",         
    colormodel = "cmyk")
